"""
BaseScratchPadManager — 通用文件暂存管理器基类

设计理念（Filesystem Context Engineering）：
- Agent 执行完后，完整结果写入文件，prompt 只传摘要和文件路径
- 写一次，按需读：各 Agent 按需读取，不通过消息链传递大数据
- 文件即接口：Agent 之间不直接传递大数据，通过文件路径引用

此基类提供场景无关的通用文件暂存能力：
- 结果文件写入/读取
- Manifest 索引管理
- 摘要生成（可覆写 _generate_summary）
- Plan 持久化
- 文件生命周期管理（过期清理）

场景特定功能（如 DB 临时表管理）由子类扩展。
"""
...
class BaseScratchPadManager:
    """
    通用文件暂存管理器基类

    核心职责：
    1. 将执行结果写入 JSON 文件，返回带 _file_ref 的精简版结果
    ...
    def save_task_result(
        ...
        full_result = convert_to_json_serializable(full_result)
